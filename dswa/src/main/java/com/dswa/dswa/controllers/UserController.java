...
public class UserController {
    ...
    public ResponseEntity<?> createUser(@RequestBody UserModel user) {
        ...
        boolean validatePassword = this.userService.Validepassword(user);

        if (doubleEmails) {
            return ResponseEntity.badRequest().body("This user already exists! Please consider logging in.");
        }
        else if(!validatePassword){
            return ResponseEntity.badRequest().body("This password does not succeed the password standards");
        }
